...
def fetch_historical_btc_pairs_asynchronously(exchanges_list,number_of_exchange):
    ...
    path_to_async_ohlcv_db=os.path.join ( os.getcwd () ,
                                         ...
                                         f"{number_of_exchange}_async_all_exchanges_multiple_tables_historical_data_for_btc_trading_pairs.db" )
...
if __name__=="__main__":
    list_of_all_exchanges=ccxt.exchanges
    how_many_exchanges=len(list_of_all_exchanges)
    try:
        process_list = []
        ...
        #     print (
        #         f"exchanges[{exchange_counter}:{exchange_counter} +"
        #         f" {how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange}]" )
        ...
        for number_of_exchange , exchange \
                in enumerate ( list_of_all_exchanges[0:30] ):
            p = multiprocessing.Process ( target =
                                          fetch_historical_btc_pairs_asynchronously ,
                                          args = (list_of_all_exchanges[0:10],number_of_exchange ))
            p.start ()
