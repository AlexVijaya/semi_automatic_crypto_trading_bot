...
def find_mirror_levels_in_database():
    ...
    try:
        drop_table ( "mirror_levels_calculated_separately_last_bar_included" ,
        ...
        try:
            ...
            for row_number_in_highs in range ( last_several_days_highs_slice_Series.size ):
                for row_number_in_lows in range ( last_several_days_lows_slice_Series.size ):
                    ...
                    # print ( row_number_in_highs )
                    # print(f"daily_low for {usdt_pair} on {exchange}")
                    # print ( daily_low )
                    ...
                    if daily_high == daily_low:
                        ...
                        if row_number_in_lows==last_several_days_lows_slice_Series.size-1:
                            ...
                            prev_daily_high = last_several_days_highs_slice_Series.iloc[row_number_in_highs - 1]
                            next_daily_high = last_several_days_highs_slice_Series.iloc[row_number_in_highs+1]
                            prev_daily_low = last_several_days_lows_slice_Series.iloc[
                                row_number_in_lows - 1]

                            if prev_daily_high < daily_high and next_daily_high < daily_high:
                                print ( "found legit high" )
                                if prev_daily_low > daily_low :
                                    ...
                                    mirror_level_df.to_sql ( "mirror_levels_calculated_separately_last_bar_included" ,
                                                             connection_to_btc_and_usdt_trading_pairs ,
                                                             if_exists = 'append' , index = False )

                        if row_number_in_highs==last_several_days_lows_slice_Series.size-1:
                            print(f"high_is_the_last_in_ohlcv_table with daily low = {daily_high}")
                            prev_daily_low = last_several_days_lows_slice_Series.iloc[
                                row_number_in_lows - 1]
                            next_daily_low = last_several_days_lows_slice_Series.iloc[row_number_in_lows+1]
                            prev_daily_high = last_several_days_highs_slice_Series.iloc[row_number_in_highs - 1]
                            if prev_daily_low > daily_low and next_daily_low > daily_low:
                                print ( "found legit low" )
                                if prev_daily_high < daily_high :
                                    print ( "level is legit\n" )
