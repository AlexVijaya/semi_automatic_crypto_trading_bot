            ...
            try:
                ...
                if "/USDT" in trading_pair:
                    ...
                    if ('active' in exchange_object.markets[trading_pair]) or (exchange_object.markets[trading_pair]['active']):
                        ...
                        if not abs(current_timestamp-last_timestamp_in_df)<(86400*1):
                        ...
                        ########################################
                        ##########################################
                        round_to_this_number_of_decimals=2
                        ###################################
                        ###################################

                        data_df['open'] = data_df['open'].round (
                            decimals = round_to_this_number_of_decimals )
                        data_df['high'] = data_df['high'].round (
                            decimals = round_to_this_number_of_decimals )
                        data_df['low'] = data_df['low'].round (
                            decimals = round_to_this_number_of_decimals )
                        data_df['close'] = data_df['close'].round (
                            decimals = round_to_this_number_of_decimals )
                        print ( data_df )
                        data_df.to_sql ( f"{trading_pair}_on_{exchange}" ,
                                                 engine_for_rounded_usdt_trading_pairs_ohlcv_db ,
...
def fetch_historical_usdt_pairs_asynchronously(engine,
                                               engine_for_rounded_usdt_trading_pairs_ohlcv_db,
                                               exchanges_list):
    ...
    tasks=[loop.create_task(get_hisorical_data_from_exchange_for_many_symbols(exchange,
                                                                              counter,
                                                                              engine,
                                                                              engine_for_rounded_usdt_trading_pairs_ohlcv_db)) for exchange in  exchanges_list]
...
def fetch_all_ohlcv_tables():
    engine , connection_to_ohlcv_for_usdt_pairs = connect_to_postres_db ()
    engine_for_rounded_usdt_trading_pairs_ohlcv_db , \
    connection_to_rounded_usdt_trading_pairs_ohlcv = connect_to_postres_db_with_dropping_it_first ( "async_rounded_ohlcv_data_for_usdt_trading_pairs" )
    ...
    for exchange_counter in \
        ...
        p = multiprocessing.Process ( target =
                                      ...
                                      args = (engine ,engine_for_rounded_usdt_trading_pairs_ohlcv_db, exchanges_list[
                                                       exchange_counter:exchange_counter + step_for_exchanges]) )
        p.start ()
