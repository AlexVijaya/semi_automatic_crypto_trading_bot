...
# list_of_exchanges=get_list_of_all_exchanges()
# for exchange_name in list_of_exchanges:
#
#     try:
#         get_USDT_and_BTC_trading_pairs_from_one_exchange(exchange_name)
#     except Exception as e:
#         print(f"problem with {exchange_name}")
...
def get_USDT_and_BTC_trading_pairs_from_all_exchanges():
    ...
    for exchange in list_of_all_exchanges:
        ...
        Path ( path_to_databases ).mkdir ( parents = True , exist_ok = True )
        # path_to_db = os.path.join ( os.getcwd () , "datasets" ,
        #                             "sql_databases" ,"multiproc_databases"
        #                             "all_exchanges_multiple_tables_historical_data_for_btc_trading_pairs.db" )
...
def place_ohlcv_from_exchange_into_seperate_db(exchange,number_of_exchange):
    list_of_all_usdt_pairs_from_all_exchanges=[]
    list_of_all_exchanges = get_list_of_all_exchanges ()
    path_to_multiproc_ohlcv_db = os.path.join ( os.getcwd () ,
                                                "datasets" ,
                                                "sql_databases" , "multiproc_databases" ,
                                                f"{number_of_exchange}_multiproc_{exchange}_multiple_tables_historical_data_for_usdt_trading_pairs.db" )
    ...
    #                                       'datasets' ,
    #                                       'plots' ,
    #                                       'crypto_exchange_plots_levels_formed_by_last_three_or_two_lows' )

    # try:
    #     shutil.rmtree ( path_to_async_ohlcv_db_dir )
    #     pass
    # except Exception as e:
    ...
    connection_to_usdt_trading_pairs_ohlcv = \
        sqlite3.connect ( path_to_multiproc_ohlcv_db )
...
def get_BTC_and_USDT_pair_ohlcv_from_exchanges(list_of_all_btc_pairs_from_all_exchanges_without_duplicates,
    ...
    try:
        ...
        print ( "error deleting folder \n" , e )
        pass
    number_of_exchanges=len(list_of_all_exchanges)
    process_list=[]
    how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange =\
        len ( list_of_all_exchanges ) // 4
    print ( "how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange=" ,
            how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange )
    for exchange_counter in \
            range(0,len(list_of_all_exchanges),
                  how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange):
        print("exchange_counter=",exchange_counter)
        print (
            f"exchanges[{exchange_counter}:{exchange_counter} + {how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange}]" )
        print ( list_of_all_exchanges[
                exchange_counter:exchange_counter + how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange] )
        for number_of_exchange, exchange \
                in enumerate(list_of_all_exchanges[exchange_counter:exchange_counter +
                                                                    how_many_separate_processes_to_spawn_each_corresponding_to_one_exchange] ):
            p=multiprocessing.Process(target =
                                      place_ohlcv_from_exchange_into_seperate_db,
                                      args=(exchange,number_of_exchange))
            p.start()
        ...
        # place_ohlcv_from_exchange_into_seperate_db(exchange,number_of_exchange)
